package com.weile.casualgames.view.widget.radarscan;
...
public class RadarViewGroup extends ViewGroup {
    ...
    private com.weile.casualgames.view.widget.radarscan.CircleView currentShowChild;//当前展示的item
    private com.weile.casualgames.view.widget.radarscan.CircleView minShowChild;//最小距离的item
...
//            if (child.getId() == R.id.id_scan_circle) {
                //为雷达扫描图设置需要的属性
//                ((com.weile.casualgames.view.widget.radarscan.RadarView) child).setScanningListener(this);
                //考虑到数据没有添加前扫描图在扫描，但是不会开始为CircleView布局
//                if (mDatas != null && mDatas.size() > 0) {
//                    ((com.weile.casualgames.view.widget.radarscan.RadarView) child).setMaxScanItemCount(mDatas.size());
//                    ((com.weile.casualgames.view.widget.radarscan.RadarView) child).startScan();
//                }
                continue;
//            }
        }
...
//            ((com.weile.casualgames.view.widget.radarscan.CircleView) child).setDisX((float) Math.cos(Math.toRadians(scanAngleList.get(i - 1) - 5))
//                    * ((com.weile.casualgames.view.widget.radarscan.CircleView) child).getProportion() * mWidth / 2);
//            ((com.weile.casualgames.view.widget.radarscan.CircleView) child).setDisY((float) Math.sin(Math.toRadians(scanAngleList.get(i - 1) - 5))
//                    * ((com.weile.casualgames.view.widget.radarscan.CircleView) child).getProportion() * mWidth / 2);
...
//            }
            //放置Circle小圆点
            child.layout((int) ((com.weile.casualgames.view.widget.radarscan.CircleView) child).getDisX() + mWidth / 2, (int) ((com.weile.casualgames.view.widget.radarscan.CircleView) child).getDisY() + mHeight / 2,
                    (int) ((com.weile.casualgames.view.widget.radarscan.CircleView) child).getDisX() + child.getMeasuredWidth() + mWidth / 2,
                    (int) ((com.weile.casualgames.view.widget.radarscan.CircleView) child).getDisY() + child.getMeasuredHeight() + mHeight / 2);
            //设置点击事件
            child.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    resetAnim(currentShowChild);
                    currentShowChild = (com.weile.casualgames.view.widget.radarscan.CircleView) child;
    ...
    public void setDatas(SparseArray<Info> mDatas) {
        ...
        for (int i = 0; i < dataLength; i++) {
            com.weile.casualgames.view.widget.radarscan.CircleView circleView = new com.weile.casualgames.view.widget.radarscan.CircleView(getContext());
            if (mDatas.get(i).getSex()) {
//                circleView.setPaintColor(getResources().getColor(R.color.bg_color_pink));
                circleView.setPortraitIcon(R.mipmap.ic_default_avatar);
            } else {
...
//        LogUtil.m("完成回调");
     ...
     * @param object
     */
    private void resetAnim(com.weile.casualgames.view.widget.radarscan.CircleView object) {
        if (object != null) {
     ...
     * @param position
     */
    private void startAnim(com.weile.casualgames.view.widget.radarscan.CircleView object, int position) {
        if (object != null) {
    ...
    public void setCurrentShowItem(int position) {
        com.weile.casualgames.view.widget.radarscan.CircleView child = (com.weile.casualgames.view.widget.radarscan.CircleView) getChildAt(position + 1);
        resetAnim(currentShowChild);
